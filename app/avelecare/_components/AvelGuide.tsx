...
// AvelGuide ("Ava") — Eastside Hospital-branded in-product assistant.
//
// Visible on every Eastside Hospital portal page. Purpose: explain pages
...
// branded for Eastside Hospital and with Eastside Hospital-specific flows. Easy to swap
...
const SCRIPT: Record<StepId, Step> = {
  ...
  "feedback-thanks": {
    message:
      "Thank you — your feedback is logged with the page you were on. Your Eastside Hospital portal team reviews these weekly.",
    options: [
      { label: "Back to menu", next: "menu" },
    ],
  },
  "tour-1": {
    message:
      "Welcome to the Eastside Hospital Credentialing Portal. I can give you a 5-step tour in about 90 seconds. Step 1 — the Dashboard. This is your real-time pipeline. KPI tiles show pipeline volume, ready-to-work, ready-to-bill, and average days to credential. The pipeline visual breaks every provider into their current stage.",
    options: [
  ...
  "tour-2": {
    message:
      "Step 2 — Providers. Click 'Providers' in the left nav. This is the full Eastside Hospital clinician roster — Emergency, ICU, Hospitalist, Behavioral Health, EMS, Pharmacy, Senior Care, School Health, and Specialty Clinic. Filter by service line, by space, by readiness state.",
    options: [
  ...
  "tour-3": {
    message:
      "Step 3 — Spaces & Programs. Eastside Hospital organizes credentialing by facility and program — East Adams Rural Healthcare, the Minnesota Rural Hospital Network, Kansas RHTP, NY Crisis Care, the National School Health Network, and Senior Care. Each space tells you how many clinicians are ready to bill there.",
    options: [
  ...
  "tour-4": {
    message:
      "Step 4 — Workflows. The credentialing templates Eastside Hospital uses across service lines. Emergency, ICU, Hospitalist, Behavioral Health, School Health, Rural Health — each has its own stages, SLA targets, and ownership. Assigning the right workflow keeps every new clinician on the same path.",
    options: [
  ...
  "tour-done": {
    message:
      "That's the tour. The portal is built around two questions you'll keep asking — 'who is ready to work?' and 'who is ready to bill?' Everything else exists to answer those two faster. Welcome to Eastside Hospital Credentialing.",
    options: [
      { label: "Back to menu", next: "menu" },
    ],
  },
...
function blockerExplanation(query: string): string {
  ...
  if (p.readyToBill) {
    return `${p.name} is fully ready to bill — no blockers. They're billable for all assigned Eastside Hospital spaces.`;
  }
  if (p.flags.length === 0) {
...
export function AvelGuide() {
  ...
  return (
    <div className={`avel-guide${minimized ? " is-minimized" : ""}`} role="dialog" aria-label="Ava — Eastside Hospital credentialing assistant">
      ...
      {!minimized && (
        <>
          ...
          <div className="avel-guide-foot">
            Eastside Hospital · in-product assistant
          </div>
        </>
      )}
    </div>
