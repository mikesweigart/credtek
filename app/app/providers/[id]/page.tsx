// Provider credentialing workspace — full lifecycle, live + persistent.
// Animated progress (with SLA red flags + automation/follow-up timing),
// then tabbed management of licenses, credentials, documents, and payer
// enrollments. Role-aware: read-only roles (clients) see status without
// the edit controls.
...
export default async function ProviderWorkspace(props: { params: Promise<{ id: string }> }) {
  ...
  const [p, ctx] = await Promise.all([getProviderById(id), getSessionContext()]);
  ...
  const summary = (
    <div className="portal-card">
      <div className="portal-detail-grid">
        ...
        <div><dt>Licenses</dt><dd>{licenses.length}</dd></div>
        ...
        <ul className="portal-rows">
          {licenses.map((l) => (
            <li key={l.id} className="portal-row">
              ...
              {editor && <DeleteBtn table="provider_licenses" id={l.id} providerId={p.id} />}
            </li>
          ))}
        </ul>
      )}
        ...
        <ul className="portal-rows">
          {documents.map((d) => (
            <li key={d.id} className="portal-row">
              ...
              {editor && <DeleteBtn table="documents" id={d.id} providerId={p.id} />}
            </li>
          ))}
        </ul>
      )}
        ...
        <ul className="portal-rows">
          {enrollments.map((e) => (
            <li key={e.id} className="portal-row">
              ...
              {editor && <DeleteBtn table="enrollments" id={e.id} providerId={p.id} />}
            </li>
          ))}
        </ul>
      )}
  ...
  return (
    <div>
      ...
      {/* Credentialing progress — animated, with red flags + automation */}
      <div className="portal-card">
        <div className="portal-card-head">
          <h2>Credentialing progress</h2>
